...
# %% auto 0
__all__ = ['Event', 'EventGroup', 'Direction', 'Data', 'clip_df_times', 'Lobster', 'MPLobster', 'DCMPLobster', 'EhMPLobster',
           'OldMPLobster', 'load_lobster', 'load_lobsters', 'infer_ticker_to_date_range', 'FolderInfo',
           'infer_ticker_dict', 'infer_ticker_to_ticker_path']
...
from typing import Literal, Optional, Self, Protocol, runtime_checkable, get_args, cast
...
# %% ../notebooks/01_preprocessing.ipynb 7
@enum.unique
...
class EventGroup(enum.Enum):
    EXECUTIONS = [
        ...
        # auction has no price or volume data
...
# %% ../notebooks/01_preprocessing.ipynb 13
@enum.unique
...
class Data:
    ...
    def __post_init__(self) -> None:
        ...
        self.levels = cast(int, self.levels)
...
# %% ../notebooks/01_preprocessing.ipynb 18
...
class MPLobster:
    ...
    def process_all_books(book_paths: dict, max_workers=70):
        ...
        del dfs
        # df = pd.concat(
        #     (self.process_books(filename) for filename in self.data.book_paths.values())
        # )
        # hm for now just don't grab message index..
        # df.set_index(self.messages.index, inplace=True, drop=True)
        price_cols = df.columns.str.contains("price")
        df.loc[:, price_cols] = df.loc[:, price_cols] / 10_000
        return df
...
class DCMPLobster:
    ...
    #         dfs = list(executor.map(self.process_messages, self.data.messages_paths.keys(), self.data.messages_paths.values()))
    #         # dfs = (self.process_messages(date=date, filepath=filepath) for date, filepath in self.data.messages_paths.items())
    #     df = pd.concat(dfs)
    ...
    #     # print(self.MAX_WORKERS)
    #     with ProcessPoolExecutor(max_workers=MPLobster.MAX_WORKERS) as executor:
    #         dfs = list(executor.map(MPLobster.process_messages, MPLobster.data.messages_paths.keys(), self.data.messages_paths.values()))
    #         # dfs = (self.process_messages(date=date, filepath=filepath) for date, filepath in self.data.messages_paths.items())
    #     df = pd.concat(dfs)
    ...
    #     print(self.MAX_WORKERS)
    #     with ProcessPoolExecutor(max_workers=self.MAX_WORKERS) as executor:
    #         dfs = list(executor.map(self.process_books, self.data.book_paths.values()))
    #     df = pd.concat(dfs)
    #     # df = pd.concat(
    #     #     (self.process_books(filename) for filename in self.data.book_paths.values())
    #     # )
    ...
    def __post_init__(self):
        ...
        if self.data.load_messages:
            self.messages = self.process_all_messages()

        if self.data.load_book:
            self.book = self.process_all_books()
...
# %% ../notebooks/01_preprocessing.ipynb 25
# | code-fold: true
@dataclass
class EhMPLobster:
    "MAN! still weirdddddd....Try again.......This is the most confusing thing EVER...Exactly the same except wrap the csv loading in mp."
    MAX_WORKERS=70
    data: Data | None = None

    def simple_echo(self, date):
        # print(f"start {date}")
        # time.sleep(10)
        # print(f"end {date}")
    ...
    def process_book(self, filepath):
        print(f"starting BOOK {filepath}")
        df = pd.read_csv(
            filepath,
            header=None,
            nrows=self.data.nrows,
            usecols=list(range(4 * self.data.levels)),
            names=self.book_col_names,
            dtype="float64",
            na_values=["-9999999999", "9999999999", "0"],
        )
        print(f"ending BOOK {filepath}")
        return df
    ...
    def __post_init__(self):
        ...
        print(self.data)
        print("====================================")
        print(self.data.load_messages)
        print(self.data.load_book)
        print("====================================")

        if self.data.load_messages:

            dates, filepaths = zip(*self.data.messages_paths.items())
            ##########################################################
            ##########################################################
            
            # ok this works fine...
            t0 = time.time()
            print('entering dummy MP of message csvs')
            with ProcessPoolExecutor(max_workers=self.MAX_WORKERS) as executor:
                echo_stuff = list(executor.map(self.simple_echo, dates))
            t1 = time.time()
            print(t1 - t0)
            ##########################################################
            ##########################################################
            print(type(filepaths))
            print('entering mp of message csvs')
            # with ProcessPoolExecutor(max_workers=self.MAX_WORKERS, ) as executor:
            #     dfs = list(executor.map(self.process_messages, dates, filepaths))
            p = ProcessPoolExecutor(max_workers=self.MAX_WORKERS, )
            dfs = list(p.map(self.process_messages, dates, filepaths))
            df = pd.concat(dfs)
            del dfs
            self.messages = df
            ...
            del p
            # TRYING TO DEBUG THE MOST FRUSTRATING ERROR EVER
            # executor.shutdown(wait=True)  # Explicitly shut down
            # del executor
        ...
        if self.data.load_book:
            ...
            ###########################################################
            ###########################################################
            # wow wtffff, this runs in seriall...
            _, filepaths = zip(*self.data.book_paths.items())
            ...
            t0 = time.time()
            print(f"=======================================")
            print('entering dummy MP of book csvs')
            with ProcessPoolExecutor(max_workers=self.MAX_WORKERS) as executor3:
                echo_stuff = list(executor3.map(self.simple_echo, filepaths))
            t1 = time.time()
            print(t1 - t0)
            ...
            print(type(filepaths))
            print(f"=======================================")
            print(f"filepaths to iterate OVER")
            print(filepaths)
            print(f"=======================================")
            with ProcessPoolExecutor(max_workers=self.MAX_WORKERS) as executor2:
                print(executor2)
                print(executor2._queue_count)
                print(executor2._processes)
                dfs2 = list(executor2.map(self.process_book, filepaths))
            df = pd.concat(dfs2)
            del dfs2
# 

            # now use multiprocessing, but memory use?? eh?
            # dates, filepaths = zip(*self.data.messages_paths.items())
            # print('entering mp of message csvs')
            # with ProcessPoolExecutor(max_workers=self.MAX_WORKERS) as executor:
            #     dfs = list(executor.map(self.process_messages, dates, filepaths))
            # df = pd.concat(dfs)
# 
            # oh lol can't load just the book then
            df.set_index(self.messages.index, inplace=True, drop=True)
...
@dataclass
class OldMPLobster:
    "MAN! This is the most confusing thing EVER...Exactly the same except wrap the csv loading in mp."
    MAX_WORKERS=70
    data: Data | None = None
...
# %% ../notebooks/01_preprocessing.ipynb 32
...
# %% ../notebooks/01_preprocessing.ipynb 38
...
# %% ../notebooks/01_preprocessing.ipynb 39
@dataclass
class FolderInfo:
    full: str
    ticker: str
...
# %% ../notebooks/01_preprocessing.ipynb 41
